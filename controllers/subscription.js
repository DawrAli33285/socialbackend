...
module.exports.makeOneTimePayment = async(req, res) => {
  const { paymentMethod, price, creatorid, creatorname, date, time, promo } = req.body;
  const numericValue = parseFloat(price);

  try {
      // Create Stripe customer
      const customer = await stripe.customers.create({
          name: req.user.alreadyexists.username,
          email: req.user.alreadyexists.email,
      ...
      if (paymentIntent.status === 'succeeded') {
          // Your business logic after successful payment

          // Create audio response and notification
          const parsedDate = moment.utc(date, 'DD/MM/YYYY');
          if (!parsedDate.isValid()) {
              throw new Error('Invalid date format');
          }
      
          let audioresponse = await audioModel.create({
            creator: creatorid,
            user: req.user.alreadyexists._id,
            date: parsedDate.toISOString(),
            time
          });
          
       
          let reason = `You have just reserved a call with ${creatorname}`;
...
module.exports.subscribe=async(req,res)=>{
  const {paymentMethod,price,creatorid,creatorname,promo}=req.body;
  
  const expirationDate = moment().add(30, 'days').valueOf(); // .valueOf() gives milliseconds
  ...
  const numericValue = parseFloat(price);
    ...
    const price = await stripe.prices.create({
        currency: 'eur',
        unit_amount: numericValue*100,
        recurring: {
          interval: 'month',
        },
        product_data: {
          name: `camron ${creatorname}`,
        },
      });
...
if (subscription.status === 'active') {
    ...
    let subresponse=await subModel.create({
      subscriber:req.user.alreadyexists._id,
      creator:creatorid,
      price:numericValue,
      promo,
      expiray:expirationDate,
...
module.exports.subwebhook=(req,res)=>{
  ...
  const endpointSecret = "whsec_a3110fca5e6a0f7076ed5237c849dc7d559326a940fd9850bdd37b700abe6e3c";
...
module.exports.getSubscription=async(req,res)=>{
  try{
let response=await subModel.find({subscriber:req.user.alreadyexists._id}).populate('creator').populate('subscriber')
return res.status(200).json({
response  
})
  }catch(error){
